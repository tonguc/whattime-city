...
function getSETime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: SE_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: SE_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: SE_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'CET',
  }
}
const ZONES = [
...
export default function SwedenClockClient() {
  const [se, setSe] = useState({ time: '--:--:--', date: '', tzAbbr: 'CET' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('SE', 'sm')} alt="SE flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Sweden · {mounted ? se.tzAbbr : 'CET'}</div>
            <div className={`text-xs ${ts}`}>{mounted && se.tzAbbr === 'CEST' ? 'CEST · UTC+2 · Central European Summer Time' : 'CET · UTC+1 · Central European Time'}</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? se.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? se.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Sweden vs World Cities</h2>
