...
export default function CSTtoMST() {
  return (
    <ContentPageWrapper>
      ...
      <h1 className="text-3xl sm:text-4xl font-bold mb-2 text-slate-800 dark:text-white">CST to MST Converter</h1>
      <p className="text-lg text-slate-600 dark:text-slate-300 mb-6">Central Standard Time → Mountain Standard Time · CST is <strong>1 hour ahead</strong> of MST</p>
      <TZPairClient config={config} />
      <section className="mt-4 mb-4">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
          <h2 className="text-xl font-semibold text-slate-800 dark:text-white mb-4">CST vs MST — What You Need to Know</h2>
          <div className="space-y-3 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
            <p><strong className="text-slate-700 dark:text-slate-200">CST (Central Standard Time)</strong> is UTC-6, covering Illinois, Texas, Minnesota and Louisiana. In summer it becomes <strong>CDT (UTC-5)</strong>.</p>
            <p><strong className="text-slate-700 dark:text-slate-200">MST (Mountain Standard Time)</strong> is UTC-7, covering Colorado, Utah, Montana and New Mexico. In summer it becomes <strong>MDT (UTC-6)</strong>.</p>
