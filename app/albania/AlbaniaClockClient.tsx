...
function getLocalTime() {
  ...
  return {
    ...
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: AL_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'CET',
  }
}
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('AL', 'sm')} alt="AL flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Albania · {mounted ? local.tzAbbr : 'CET'}</div>
      ...
      <ClockComparisonSection primaryTz={AL_TZ} countryName="Albania" />
    </div>
  )
}
