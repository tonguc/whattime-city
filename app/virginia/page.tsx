...
export default function VirginiaTimePage() {
  return (
    <ContentPageWrapper>
      ...
      <HubPageHeader title="Current Time in Virginia" subtitle="Eastern Time (ET) · EST (UTC−5) in winter · EDT (UTC−4) during Daylight Saving Time · DC area" />
      <VirginiaClockClient />
            <HubPageLayout
        faqItems={faqSchema.mainEntity.map(i => ({ name: i.name, text: i.acceptedAnswer.text }))}
        links={[{ label: 'Richmond time', href: '/richmond/' }, { label: 'Virginia Beach time', href: '/virginia-beach/' }, { label: 'Arlington time', href: '/arlington/' }, { label: 'Richmond → Chicago', href: '/time/richmond/chicago/' }, { label: 'Richmond → Los Angeles', href: '/time/richmond/los-angeles/' }, { label: 'Richmond → London', href: '/time/richmond/london/' }, { label: 'Time converter tool', href: '/time-converter/' }]}
        linksTitle="Virginia City Times & Converters"
        footerText="Time zone data powered by the IANA Time Zone Database. Virginia: America/New_York (EST UTC−5 / EDT UTC−4)."
      />
    </ContentPageWrapper>
  )
}
