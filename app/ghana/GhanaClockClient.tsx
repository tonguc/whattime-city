...
export default function GhanaClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Africa/Accra', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
    }
    update()
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-emerald-600">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            <span className="px-3 py-1 rounded-full font-medium bg-yellow-400/40">GMT · UTC+0 (always)</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">No DST — Near Equator</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Accra</span>
          </div>
        </div>
      </section>
