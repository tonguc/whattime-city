      ...
      <section className={`${cardClass} mb-4`}>
        ...
        <div className={`space-y-4 ${theme.textMuted}`}>
          <div>
            ...
            <p className="text-sm">Yes. The converter automatically adjusts for DST based on each city's current rules.</p>
          </div>
          <div>
            <h3 className={`font-medium mb-1 ${theme.text}`}>How accurate is the time conversion?</h3>
            <p className="text-sm">Conversions are based on the IANA time zone database, the same source used by operating systems worldwide.</p>
          </div>
          <div>
            <h3 className={`font-medium mb-1 ${theme.text}`}>Can I convert time for cities not listed?</h3>
            <p className="text-sm">We support over 150 major cities. Use the World Clock to find cities in the same time zone as your target.</p>
          </div>
        </div>
      </section>
    </ToolPageWrapper>
