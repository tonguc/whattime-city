...
export default function WorldAlarmPage() {
  ...
  return (
    <>
      ...
      {/* Main Card */}
      <div className={`rounded-2xl p-8 mb-8 backdrop-blur-xl ${theme.card} text-center`}>
      ...
      {/* SEO Content */}
      <section className={`rounded-2xl p-6 mb-6 backdrop-blur-xl ${theme.card}`}>
      ...
      {/* Related Tools */}
      <section className={`rounded-2xl p-6 mb-8 backdrop-blur-xl ${theme.card}`}>
        <h2 className={`text-xl font-semibold mb-4 ${theme.text}`}>
          Related Tools
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Link href="/converter" className={`p-4 rounded-xl transition-all hover:scale-[1.02] ${
            isLight ? 'bg-slate-50 hover:bg-slate-100' : 'bg-slate-700/60 hover:bg-slate-700/80'
          }`}>
