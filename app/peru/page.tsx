...
export default function PeruTimePage() {
  return (
    <ContentPageWrapper>
      ...
      <HubPageHeader title="Current Time in Peru" subtitle="Peru Time (PET) · UTC−5 · No Daylight Saving Time" />
      <PeruClockClient />
      <CountryFactsSection hubSlug="peru" />
            <HubPageLayout
        faqItems={faqSchema.mainEntity.map(i => ({ name: i.name, text: i.acceptedAnswer.text }))}
        links={[{ label: 'Lima time', href: '/lima/' }, { label: 'Bogotá time', href: '/bogota/' }, { label: 'Buenos Aires time', href: '/buenos-aires/' }, { label: 'Time in Colombia', href: '/colombia/' }, { label: 'Time in Argentina', href: '/argentina/' }, { label: 'Time in Chile', href: '/chile/' }, { label: 'Lima → New York', href: '/time/lima/new-york/' }, { label: 'Time converter tool', href: '/time-converter/' }]}
        linksTitle="Peru & Nearby Times"
        footerText="Time zone data powered by the IANA Time Zone Database. Peru: America/Lima (PET UTC−5, no DST)."
      />
    </ContentPageWrapper>
  )
}
