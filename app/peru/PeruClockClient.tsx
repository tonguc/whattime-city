...
function getPETime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: PE_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: PE_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: PE_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'PET',
  }
}
const ZONES = [
...
export default function PeruClockClient() {
  const [pe, setPe] = useState({ time: '--:--:--', date: '', tzAbbr: 'PET' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('PE', 'sm')} alt="PE flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Peru · PET · UTC−5</div>
            <div className={`text-xs ${ts}`}>PET · UTC−5 · Peru Time · No DST</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? pe.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? pe.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Peru vs Other Cities</h2>
