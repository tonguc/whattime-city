...
export default function BelgiumClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'Europe/Brussels', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Europe/Brussels', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
      const jan = new Date(now.getFullYear(), 0, 1)
      const janStr = jan.toLocaleString('en-US', { timeZone: 'Europe/Brussels' })
      const nowStr = now.toLocaleString('en-US', { timeZone: 'Europe/Brussels' })
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-yellow-700">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            ...
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Capital of Europe</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Brussels</span>
          </div>
        </div>
      </section>
