...
export default function ParaguayClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'America/Asuncion', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'America/Asuncion', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
      const jan = new Date(now.getFullYear(), 0, 1)
      const janStr = jan.toLocaleString('en-US', { timeZone: 'America/Asuncion' })
      const nowStr = now.toLocaleString('en-US', { timeZone: 'America/Asuncion' })
        ...
        <div className="rounded-2xl text-white p-6 text-center bg-red-700">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            <span className="px-3 py-1 rounded-full font-medium bg-blue-400/40">
              {isDST ? 'PYST · UTC-3' : 'PYT · UTC-4'}
            </span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Southern Hemisphere DST</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Asunci&oacute;n</span>
          </div>
        </div>
      </section>
