...
export default function RomaniaClockClient() {
  const { time, date, mounted, isDST } = useClockState('Europe/Bucharest')
  ...
  return (
    <div className="space-y-4">
      <ClockHero
        ...
        clocks={[{ label: 'Current Time in Romania', time }]}
        date={date}
        mounted={mounted}
        badges={[
          { label: isDST ? 'EEST · UTC+3' : 'EET · UTC+2' },
          { label: 'Europe&apos;s IT Outsourcing Star', highlight: true },
          { label: 'Bucharest' },
        ]}
      />
