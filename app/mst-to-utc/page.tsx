...
export default function MSTtoUTC() {
  return (
    <ContentPageWrapper>
      ...
      <h1 className="text-3xl sm:text-4xl font-bold mb-2 text-slate-800 dark:text-white">MST to UTC Converter</h1>
      <p className="text-lg text-slate-600 dark:text-slate-300 mb-6">Mountain Standard Time → Coordinated Universal Time · MST is <strong>7 hours behind</strong> UTC</p>
      <TZPairClient config={config} />
      <section className="mt-4 mb-4">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-6">
          <h2 className="text-xl font-semibold text-slate-800 dark:text-white mb-4">MST vs UTC — What You Need to Know</h2>
          <div className="space-y-3 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
            <p><strong className="text-slate-700 dark:text-slate-200">MST (UTC-7)</strong> covers the Rocky Mountain US states. In summer it becomes <strong>MDT (UTC-6)</strong> — except Arizona (no DST, always UTC-7).</p>
            <p><strong className="text-slate-700 dark:text-slate-200">UTC</strong> is the global time standard used in servers and APIs. A 9 AM MST start equals 16:00 UTC in winter.</p>
          </div>
        </div>
      </section>
