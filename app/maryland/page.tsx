...
export default function MarylandTimePage() {
  return (
    <ContentPageWrapper>
      ...
      <HubPageHeader title="Current Time in Maryland" subtitle="Eastern Time (EST/EDT) · Baltimore · UTC−5 / UTC−4" />
      <MarylandClockClient />
            <HubPageLayout
        faqItems={faqSchema.mainEntity.map(i => ({ name: i.name, text: i.acceptedAnswer.text }))}
        links={[{ label: 'Baltimore time', href: '/baltimore/' }, { label: 'Washington D.C. time', href: '/washington-dc/' }, { label: 'Philadelphia time', href: '/philadelphia/' }, { label: 'Time in Virginia', href: '/virginia/' }, { label: 'Time in Pennsylvania', href: '/pennsylvania/' }, { label: 'Time in New Jersey', href: '/new-jersey/' }, { label: 'Time in Massachusetts', href: '/massachusetts/' }, { label: 'Time converter tool', href: '/time-converter/' }]}
        linksTitle="Maryland City Times & Converters"
        footerText="Time zone data powered by the IANA Time Zone Database. Maryland: America/New_York (EST/EDT, Eastern Time)."
      />
    </ContentPageWrapper>
  )
}
