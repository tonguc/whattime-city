...
const UT_TZ = 'America/Denver'
function getUTTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: UT_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: UT_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: UT_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'MST',
  }
}
const ZONES = [
...
export default function UtahClockClient() {
  const [ut, setUt] = useState({ time: '--:--:--', date: '', tzAbbr: 'MST' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Utah · {mounted ? ut.tzAbbr : 'MST'}</div>
            <div className={`text-xs ${ts}`}>{mounted && ut.tzAbbr === 'MDT' ? 'MDT · UTC−6 · Mountain Daylight Time' : 'MST · UTC−7 · Mountain Standard Time'}</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? ut.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? ut.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Utah vs Other Cities</h2>
