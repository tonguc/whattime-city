...
function getLocalTime() {
  ...
  return {
    ...
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: MD_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'EET',
  }
}
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('MD', 'sm')} alt="MD flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Moldova · {mounted ? local.tzAbbr : 'EET'}</div>
      ...
      <ClockComparisonSection primaryTz={MD_TZ} countryName="Moldova" />
    </div>
  )
}
