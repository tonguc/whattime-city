...
export default function MoldovaClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'Europe/Chisinau', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Europe/Chisinau', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
      const jan = new Date(now.getFullYear(), 0, 1)
      const janStr = jan.toLocaleString('en-US', { timeZone: 'Europe/Chisinau' })
      const nowStr = now.toLocaleString('en-US', { timeZone: 'Europe/Chisinau' })
    ...
    <div className="space-y-4">
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-emerald-600">
          <div className="text-sm font-bold uppercase tracking-widest mb-2 opacity-90">Current Time in Moldova</div>
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            ...
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Chi&#537;in&#259;u</span>
          </div>
        </div>
      </section>
