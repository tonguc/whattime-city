...
function getUATime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: UA_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: UA_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: UA_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'EET',
  }
}
const ZONES = [
  { city: 'Warsaw', tz: 'Europe/Warsaw', label: 'CET/CEST' },
...
export default function UkraineClockClient() {
  const [ua, setUa] = useState({ time: '--:--:--', date: '', tzAbbr: 'EET' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('UA', 'sm')} alt="UA flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Ukraine (Kyiv) · {mounted ? ua.tzAbbr : 'EET'}</div>
            <div className={`text-xs ${ts}`}>{mounted && ua.tzAbbr === 'EEST' ? 'EEST · UTC+3 · Eastern European Summer Time' : 'EET · UTC+2 · Eastern European Time'}</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? ua.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? ua.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Ukraine vs Other Cities</h2>
