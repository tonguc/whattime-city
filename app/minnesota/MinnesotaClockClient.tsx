...
function getMNTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: MN_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: MN_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: MN_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'CST',
  }
}
const ZONES = [
...
export default function MinnesotaClockClient() {
  const [mn, setMn] = useState({ time: '--:--:--', date: '', tzAbbr: 'CST' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Minnesota · {mounted ? mn.tzAbbr : 'CST'}</div>
            <div className={`text-xs ${ts}`}>{mounted && mn.tzAbbr === 'CDT' ? 'CDT · UTC−5 · Central Daylight Time' : 'CST · UTC−6 · Central Standard Time'}</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? mn.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? mn.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Minnesota vs Other Cities</h2>
