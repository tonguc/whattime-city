...
export default function HoursFromNowIndividualClient({
  ...
  const answerBox = isLight
    ? 'rounded-2xl bg-green-50 border border-green-100 p-6 text-center'
    : 'rounded-2xl bg-green-900/20 border border-green-700/30 p-6 text-center'
  ...
  const nextDayBox = isLight
    ? 'rounded-lg bg-blue-50 border border-blue-200 p-3 text-sm text-blue-800'
    : 'rounded-lg bg-blue-900/20 border border-blue-700/30 p-3 text-sm text-blue-300'
  ...
  const qaItems = [
    {
      q: `What time will it be in ${hours} hours?`,
      a: `In ${hours} hours it will be ${time12} (${time24}) on ${longDate}.${crossedMidnight ? ` This time is on the next calendar day (${shortDate}).` : ''}`,
    },
    {
      q: `What day will it be in ${hours} hours?`,
      a: `In ${hours} hours it will be ${dayName}, ${shortDate}.`,
    },
    {
      ...
      <div className="mx-auto max-w-3xl px-4 py-10 space-y-8">
        ...
        <div className={card}>
          ...
          <div className="space-y-3">
            {[
              { label: 'Hours from now', value: `${hours} hours` },
              { label: 'Days and hours', value: breakdown },
              { label: 'Decimal days', value: `${(hours / 24).toFixed(2)} days` },
              { label: 'Minutes from now', value: `${(hours * 60).toLocaleString()} minutes` },
            ].map(item => (
        ...
        {/* Next day note */}
        {crossedMidnight && (
          <div className={nextDayBox}>
            {hours} hours from now crosses midnight into the next day. The result is <strong>{shortDate}</strong>.
          </div>
        )}
        ...
        <div className={card}>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/hours-from-now/" className={chipBase}>Calculator</Link>
          </div>
        </div>
        ...
        <div className={cardAlt}>
          <p className={`text-sm ${muted}`}>
            Looking backward?{' '}
            <Link href={`/hours-ago/${hours}/`} className={linkStyle}>
              {hours} hours ago
            </Link>
            {' · '}
