...
export default function HowManySecondsInAYearClient() {
    ...
    <article>
      ...
      <nav className={`mb-4 text-sm ${t.breadcrumb}`} aria-label="Breadcrumb">
        <ol className="flex flex-wrap items-center gap-1">
          ...
          <li className={t.breadcrumbSep}>/</li>
          <li className={t.breadcrumbCurrent}>How Many Seconds in a Year?</li>
        </ol>
      </nav>
      ...
      <p className={`mb-8 text-lg leading-relaxed ${t.body}`}>
        There are <strong className={t.heading}>31,536,000 seconds</strong> in a regular
        year (365 &times; 24 &times; 3,600). In a leap year, there are{' '}
        <strong className={t.heading}>31,622,400 seconds</strong> (366 &times; 86,400).
      ...
      <section className="mb-10">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className={`${t.card} p-5`}>
            <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${t.muted}`}>
              Regular Year (e.g. 2026)
            </p>
            <p className={`text-3xl font-bold tabular-nums ${t.heading}`}>31,536,000</p>
            <p className={`mt-1 text-sm ${t.body}`}>seconds (365 &times; 86,400)</p>
          </div>
      ...
      <section className="mb-10">
        <h2 className={`mb-4 text-2xl font-bold ${t.heading}`}>How to Calculate Seconds in a Year</h2>
        <p className={`mb-4 leading-relaxed ${t.body}`}>
          Start from the smallest unit and multiply up:
        </p>
        <div className={`${t.cardAlt} p-5 mb-4`}>
          <div className={`space-y-3 text-sm ${t.body}`}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={t.tableHead}>
                ...
                <th className={`px-4 py-3 text-right ${t.tableHeadCell}`}>Seconds</th>
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr
                  key={year as number}
                  className={`last:border-0 ${t.tableRowBorder} ${year === 2026 ? t.tableRowCurrent : idx % 2 === 0 ? t.tableRowEven : t.tableRowOdd}`}
                >
                  ...
                  <td className={`px-4 py-3 text-right font-semibold tabular-nums ${t.heading}`}>{(seconds as number).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="mb-10">
        <h2 className={`mb-4 text-2xl font-bold ${t.heading}`}>Seconds Per Month</h2>
        <div className={t.tableWrapper}>
          <table className="w-full text-sm">
            <thead>
              <tr className={t.tableHead}>
            ...
            <tbody>
              {[
                ...
                <tr
                  key={month as string}
                  className={`last:border-0 ${t.tableRowBorder} ${idx % 2 === 0 ? t.tableRowEven : t.tableRowOdd}`}
                >
                  <td className={`px-4 py-3 ${t.heading}`}>{month}</td>
                  <td className={`px-4 py-3 text-right tabular-nums ${t.tableCell}`}>{days}</td>
                  <td className={`px-4 py-3 text-right tabular-nums ${t.tableCell}`}>{(hours as number).toLocaleString()}</td>
                  <td className={`px-4 py-3 text-right font-semibold tabular-nums ${t.heading}`}>{(seconds as number).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="mb-10">
        <h2 className={`mb-3 text-2xl font-bold ${t.heading}`}>Seconds in Other Time Units</h2>
        <div className={t.tableWrapper}>
          <table className="w-full text-sm">
            <thead>
              <tr className={t.tableHead}>
            ...
            <tbody>
              {[
                ...
                >
                  ...
                  <td className={`px-4 py-3 text-right font-semibold tabular-nums ${t.heading}`}>{seconds}</td>
      ...
      <section className="mb-10">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            <div key={item.label} className={`${t.card} p-4`}>
              <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${t.muted}`}>{item.label}</p>
              <p className={`text-xl font-bold ${t.heading}`}>{item.value}</p>
              <p className={`text-xs mt-0.5 ${t.muted}`}>{item.note}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <ul className="space-y-2 text-sm">
          <li>
            <a href="/articles/how-many-hours-in-a-year/" className={t.link}>
