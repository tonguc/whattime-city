...
function getNCTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: NC_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: NC_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: NC_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'EST',
  }
}
const ZONES = [
...
export default function NorthCarolinaClockClient() {
  const [nc, setNc] = useState({ time: '--:--:--', date: '', tzAbbr: 'EST' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>North Carolina · {mounted ? nc.tzAbbr : 'EST'}</div>
            <div className={`text-xs ${ts}`}>{mounted && nc.tzAbbr === 'EDT' ? 'EDT · UTC−4 · Eastern Daylight Time' : 'EST · UTC−5 · Eastern Standard Time'}</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? nc.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? nc.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>North Carolina vs Other Cities</h2>
