...
export default function AzerbaijanClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'Asia/Baku', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Asia/Baku', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
    }
    update()
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-sky-700">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            <span className="px-3 py-1 rounded-full font-medium bg-green-400/40">AZT · UTC+4 (always)</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">No DST since 2016</span>
            <span className="px-3 py-1 rounded-full font-medium bg-red-400/30">Land of Fire</span>
          </div>
        </div>
      </section>
