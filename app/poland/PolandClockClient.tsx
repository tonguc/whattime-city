...
function getPLTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: PL_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: PL_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: PL_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'CET',
  }
}
const ZONES = [
  { city: 'Berlin', tz: 'Europe/Berlin', label: 'CET/CEST' },
  { city: 'London', tz: 'Europe/London', label: 'GMT/BST' },
  { city: 'Kyiv', tz: 'Europe/Kyiv', label: 'EET/EEST' },
...
export default function PolandClockClient() {
  const [pl, setPl] = useState({ time: '--:--:--', date: '', tzAbbr: 'CET' })
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? pl.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? pl.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Poland vs Other Cities</h2>
