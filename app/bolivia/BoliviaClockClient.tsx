...
export default function BoliviaClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'America/La_Paz', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'America/La_Paz', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
    }
    update()
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-cyan-600">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            <span className="px-3 py-1 rounded-full font-medium bg-yellow-400/40">BOT · UTC-4 (always)</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">No DST</span>
            <span className="px-3 py-1 rounded-full font-medium bg-red-400/30">World&apos;s Highest Capital</span>
          </div>
        </div>
      </section>
