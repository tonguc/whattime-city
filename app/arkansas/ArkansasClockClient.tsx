...
function getARTime() {
  ...
  return {
    ...
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: AR_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'CST',
  }
}
const ZONES = [
  { city: 'Dallas', tz: 'America/Chicago', label: 'CST/CDT' },
...
export default function ArkansasClockClient() {
  const [ar, setAr] = useState({ time: '--:--:--', date: '', tzAbbr: 'CST' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          ...
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Arkansas · {mounted ? ar.tzAbbr : 'CST'}</div>
            <div className={`text-xs ${ts}`}>{mounted && ar.tzAbbr === 'CDT' ? 'CDT · UTC−5 · Central Daylight Time' : 'CST · UTC−6 · Central Standard Time'}</div>
          </div>
        </div>
      ...
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Arkansas vs Other Cities</h2>
