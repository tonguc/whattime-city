...
const SN_TZ = 'Africa/Dakar'
function getSNTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: SN_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: SN_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: SN_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'GMT',
  }
}
const ZONES = [
...
export default function SenegalClockClient() {
  const [sn, setSn] = useState({ time: '--:--:--', date: '', tzAbbr: 'GMT' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('SN', 'sm')} alt="SN flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Senegal · GMT · UTC+0</div>
            <div className={`text-xs ${ts}`}>GMT · UTC+0 · Greenwich Mean Time · No DST</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? sn.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? sn.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Senegal vs Other Cities</h2>
