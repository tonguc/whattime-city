  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-2xl p-5 ${card}`}>
        <h2 className={`text-lg font-semibold mb-3 ${head}`}>Dubai Time vs World Cities</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className={`divide-y ${div}`}>
              <tr>
                <td className={`py-2 pr-4 font-semibold ${head}`}>Dubai (GST)</td>
                <td className={`py-2 pr-4 ${muted}`}>UTC+4</td>
                <td className="py-2 font-mono font-bold text-orange-600">
                  {mounted ? gst.time.slice(0, 5) : '--:--'}
                </td>
              </tr>
              {WORLD_CITIES.map(c => (
                <tr key={c.name}>
