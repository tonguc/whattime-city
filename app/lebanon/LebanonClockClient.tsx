...
export default function LebanonClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'Asia/Beirut', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Asia/Beirut', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
      const jan = new Date(now.getFullYear(), 0, 1)
      const janStr = jan.toLocaleString('en-US', { timeZone: 'Asia/Beirut' })
      const nowStr = now.toLocaleString('en-US', { timeZone: 'Asia/Beirut' })
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-red-800">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            ...
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">Beirut</span>
          </div>
        </div>
      </section>
