...
function getCMTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: CM_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: CM_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: CM_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'WAT',
  }
}
const ZONES = [
...
export default function CameroonClockClient() {
  const [cm, setCm] = useState({ time: '--:--:--', date: '', tzAbbr: 'WAT' })
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('CM', 'sm')} alt="CM flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Cameroon · WAT · UTC+1</div>
            <div className={`text-xs ${ts}`}>WAT · UTC+1 · West Africa Time · No DST</div>
          </div>
        </div>
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? cm.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? cm.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Cameroon vs Other Cities</h2>
