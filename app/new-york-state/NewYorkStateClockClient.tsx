  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`rounded-2xl p-5 ${card}`}>
        <h2 className={`text-lg font-semibold mb-3 ${head}`}>New York Time vs World Cities</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className={`divide-y ${div}`}>
              <tr>
                <td className={`py-2 pr-4 font-semibold ${head}`}>New York ({mounted ? ny.tzAbbr : 'EST/EDT'})</td>
                <td className={`py-2 pr-4 ${muted}`}>{utcLabel}</td>
                <td className="py-2 font-mono font-bold text-slate-600">
                  {mounted ? ny.time.slice(0, 5) : '--:--'}
                </td>
              </tr>
              {WORLD_CITIES.map(c => (
                <tr key={c.name}>
