...
export default function OhioClockClient() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={cardBase}>
        <h2 className={`text-base font-semibold mb-3 ${textPrimary}`}>Ohio vs Other Cities</h2>
      ...
      <div className={cardBase}>
        <h2 className={`text-base font-semibold mb-3 ${textPrimary}`}>Best Time to Call Ohio (EST/EDT)</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className={`border-b ${tableRowClass}`}>
                <th className={`text-left py-2 pr-4 font-medium ${textSecondary}`}>Calling From</th>
                <th className={`text-left py-2 font-medium ${textSecondary}`}>Call during (local time) → Ohio receives 9 AM–6 PM</th>
              </tr>
            </thead>
            <tbody className={`divide-y ${tableRowClass}`}>
              {[
                { from: 'Chicago (CST, UTC−6)', window: '8 AM – 5 PM (1h behind Ohio)' },
                { from: 'Denver (MST, UTC−7)', window: '7 AM – 4 PM (2h behind Ohio)' },
                { from: 'Los Angeles (PST, UTC−8)', window: '6 AM – 3 PM (3h behind Ohio)' },
                ...
                { from: 'Tokyo (JST, UTC+9)', window: '11 PM – 8 AM next day' },
              ].map(r => (
                <tr key={r.from}>
