...
export default function LaosClockClient() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const now = new Date()
      setTime(now.toLocaleTimeString('en-US', { timeZone: 'Asia/Vientiane', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }))
      setDate(now.toLocaleDateString('en-US', { timeZone: 'Asia/Vientiane', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }))
    }
    update()
      ...
      <section>
        <div className="rounded-2xl text-white p-6 text-center bg-red-700">
          ...
          <div className="flex justify-center gap-3 text-sm flex-wrap">
            <span className="px-3 py-1 rounded-full font-medium bg-white/40">ICT · UTC+7 (always)</span>
            <span className="px-3 py-1 rounded-full font-medium bg-white/20">No DST</span>
            <span className="px-3 py-1 rounded-full font-medium bg-blue-400/30">Vientiane</span>
          </div>
        </div>
      </section>
