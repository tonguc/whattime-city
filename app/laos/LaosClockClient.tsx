...
function getLocalTime() {
  ...
  return {
    ...
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: LA_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'ICT',
  }
}
    ...
    <div className="space-y-4">
      <div className={card}>
        <div className="flex items-center gap-3 mb-1">
          <img src={getFlagUrl('LA', 'sm')} alt="LA flag" className="w-7 h-5 object-cover rounded-sm" />
          <div>
            <div className={`text-xs font-medium uppercase tracking-wider ${ts}`}>Laos · {mounted ? local.tzAbbr : 'ICT'}</div>
      ...
      <ClockComparisonSection primaryTz={LA_TZ} countryName="Laos" />
    </div>
  )
}
