...
function getKETime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: KE_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: KE_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
  }
}
const ZONES = [
...
export default function KenyaClockClient() {
  const [ke, setKe] = useState({ time: '--:--:--', date: '' })
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? ke.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? ke.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Kenya vs World Cities</h2>
