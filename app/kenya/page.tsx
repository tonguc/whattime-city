...
export default function KenyaTimePage() {
  return (
    <ContentPageWrapper>
      ...
      <HubPageHeader title="Current Time in Kenya" subtitle="East Africa Time (EAT) · UTC+3 · No Daylight Saving Time" />
      <KenyaClockClient />
      <CountryFactsSection hubSlug="kenya" />
            <HubPageLayout
        faqItems={faqSchema.mainEntity.map(i => ({ name: i.name, text: i.acceptedAnswer.text }))}
        links={[{ label: 'Nairobi time', href: '/nairobi/' }, { label: 'Mombasa time', href: '/mombasa/' }, { label: 'Nairobi → London', href: '/time/nairobi/london/' }, { label: 'Nairobi → New York', href: '/time/nairobi/new-york/' }, { label: 'Nairobi → Dubai', href: '/time/nairobi/dubai/' }, { label: 'Nairobi → Mumbai', href: '/time/nairobi/mumbai/' }, { label: 'Nairobi → Singapore', href: '/time/nairobi/singapore/' }, { label: 'Time converter tool', href: '/time-converter/' }]}
        linksTitle="Kenya City Times & Converters"
        footerText="Time zone data powered by the IANA Time Zone Database. Kenya: Africa/Nairobi (EAT UTC+3, no DST)."
      />
    </ContentPageWrapper>
  )
}
