...
export default function MoroccoTimePage() {
  return (
    <ContentPageWrapper>
      ...
      <HubPageHeader title="Current Time in Morocco" subtitle="Western European Time (WET/WEST) · UTC+0 / UTC+1 · DST suspended during Ramadan" />
      <MoroccoClockClient />
      <CountryFactsSection hubSlug="morocco" />
            <HubPageLayout
        faqItems={faqSchema.mainEntity.map(i => ({ name: i.name, text: i.acceptedAnswer.text }))}
        links={[{ label: 'Casablanca time', href: '/casablanca/' }, { label: 'London time', href: '/london/' }, { label: 'Madrid time', href: '/madrid/' }, { label: 'Paris time', href: '/paris/' }, { label: 'Time in Spain', href: '/spain/' }, { label: 'Time in Egypt', href: '/egypt/' }, { label: 'Casablanca → London', href: '/time/casablanca/london/' }, { label: 'Time converter tool', href: '/time-converter/' }]}
        linksTitle="Morocco & Nearby Times"
        footerText="Time zone data powered by the IANA Time Zone Database. Morocco: Africa/Casablanca (WET UTC+0 / WEST UTC+1, DST suspended during Ramadan)."
      />
    </ContentPageWrapper>
  )
}
