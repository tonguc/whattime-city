...
function getUZTime() {
  const now = new Date()
  return {
    time: now.toLocaleTimeString('en-US', { timeZone: UZ_TZ, hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }),
    date: now.toLocaleDateString('en-US', { timeZone: UZ_TZ, weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    tzAbbr: now.toLocaleTimeString('en-US', { timeZone: UZ_TZ, timeZoneName: 'short' }).split(' ').pop() ?? 'UZT',
  }
}
const ZONES = [
  { city: 'Almaty', tz: 'Asia/Almaty', label: 'ALMT' },
  { city: 'Astana', tz: 'Asia/Almaty', label: 'ALMT' },
  { city: 'Dubai', tz: 'Asia/Dubai', label: 'GST' },
  { city: 'Moscow', tz: 'Europe/Moscow', label: 'MSK' },
  { city: 'Karachi', tz: 'Asia/Karachi', label: 'PKT' },
  { city: 'Delhi', tz: 'Asia/Kolkata', label: 'IST' },
...
export default function UzbekistanClockClient() {
  const [uz, setUz] = useState({ time: '--:--:--', date: '', tzAbbr: 'UZT' })
  ...
  return (
    <div className="space-y-4">
      <div className={card}>
        ...
        <div className={`font-mono text-5xl sm:text-6xl font-bold tracking-tight mt-3 ${tp}`}>{mounted ? uz.time : '--:--:--'}</div>
        <div className={`text-sm mt-1 ${ts}`}>{mounted ? uz.date : ''}</div>
      </div>
      <div className={card}>
        <h2 className={`text-base font-semibold mb-3 ${tp}`}>Uzbekistan vs Other Cities</h2>
