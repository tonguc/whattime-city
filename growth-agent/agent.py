...
Faz 4.5: Decision Log + Lessons Memory + GSC Priority Queue v2
         + SERP Trigger Batch (serp_trigger_batch.py entegre)
"""
...
# SERP Trigger Batch import
try:
    from serp_trigger_batch import (
        GscRow,
        run_weekly_gsc_trigger,
        run_daily_serp_decisions,
        build_action_summary,
    )
    TRIGGER_AVAILABLE = True
except ImportError:
    TRIGGER_AVAILABLE = False
    logging.warning("serp_trigger_batch.py not found — trigger system disabled")

TRIGGER_STORE_PATH   = "trigger_state.json"
_trigger_candidates  = []   # haftalık scan sonucu burada tutulur
...
def db_record_processing(city_slug, quality_score, pr_url, issues, model, tokens, notes,
                         status="completed", failure_codes=None, what_fixed_it=None,
                         retry_count=0, last_failure_score=None, last_success_score=None):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    c.execute('''INSERT INTO city_history 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
              (city_slug, now, quality_score, pr_url, json.dumps(issues), model, tokens, notes,
               status, json.dumps(failure_codes or []), what_fixed_it or "",
    ...
    c.execute('''INSERT OR REPLACE INTO city_priority 
                 (city_slug, last_processed, process_count, avg_quality_score, updated_at,
                  priority_score, tier, continent,
                  gsc_impressions, gsc_clicks, gsc_ctr, gsc_position, gsc_top_query, gsc_updated)
                 VALUES (?, ?, 
...
# ── SERP Trigger: iki periyot GSC çekme ─────────────────────

def build_gsc_rows_for_trigger() -> list:
    """
    serp_trigger_batch.py için iki haftalık GSC verisi çeker.
    GscRow listesi döndürür.
    """
    ...
    service = get_gsc_service()
    if not service:
        logger.warning("GSC service yok — trigger rows boş")
        return []
    ...
    def pull(start, end) -> dict:
        try:
            ...
            logger.error(f"GSC pull error ({start}→{end}): {e}")
            return {}
        out = {}
        for row in resp.get("rows", []):
            page = row["keys"][0]
            slug = page.replace("https://whattime.city/", "").strip("/") or "home"
            out[slug] = {
                "clicks":      int(row.get("clicks", 0)),
                "impressions": int(row.get("impressions", 0)),
                "ctr":         round(row.get("ctr", 0) * 100, 3),
                "position":    round(row.get("position", 0), 2),
            }
        return out

    cur  = pull(start_cur,  end_cur)
    prev = pull(start_prev, end_prev)

    rows = []
    for slug in set(cur) | set(prev):
        c = cur.get(slug,  {"clicks": 0, "impressions": 0, "ctr": 0, "position": 0})
        p = prev.get(slug, {"clicks": 0, "impressions": 0, "ctr": 0, "position": 0})
        rows.append(GscRow(
            url           = f"https://whattime.city/{slug}",
            clicks_7d     = c["clicks"],
            impr_7d       = c["impressions"],
            ctr_7d        = c["ctr"],
            pos_7d        = c["position"],
            clicks_prev7d = p["clicks"],
            impr_prev7d   = p["impressions"],
            ctr_prev7d    = p["ctr"],
            pos_prev7d    = p["position"],
        ))
    logger.info(f"build_gsc_rows_for_trigger: {len(rows)} rows")
    return rows

# ── SERP Trigger: SerpApi fetch wrapper ─────────────────────

def serp_fetch_for_trigger(url: str, keyword: str) -> dict:
    config  = get_config()
    api_key = config.get("SERPAPI_KEY", "")
    if not api_key:
        logger.warning("SERPAPI_KEY eksik — SERP skipped")
        return {"top_urls": [], "features": {}}
    try:
        resp = requests.get(
            "https://serpapi.com/search",
            params={"q": keyword, "api_key": api_key, "num": 10, "gl": "us", "hl": "en"},
            timeout=15,
        )
        resp.raise_for_status()
        data = resp.json()
        top_urls = [r.get("link", "") for r in data.get("organic_results", [])[:10]]
        features = {
            "paa":              bool(data.get("related_questions")),
            "featured_snippet": bool(data.get("answer_box")),
            "local_box":        bool(data.get("local_results")),
        }
        return {"top_urls": top_urls, "features": features}
    except Exception as e:
        logger.error(f"SerpApi error ({keyword}): {e}")
        return {"top_urls": [], "features": {}}

def keyword_for_trigger_url(url: str) -> str:
...
def get_position_multiplier(position: float) -> tuple:
    if position <= 3:    return 0.6, 'top3'
    elif position <= 7:  return 1.2, 'first_page'
    elif position <= 12: return 3.5, 'push_zone_high'
    elif position <= 20: return 2.5, 'push_zone_mid'
    elif position <= 50: return 1.3, 'content_gap'
    else:                return 1.0, 'deep_gap'
...
def calculate_opportunity_score(row: dict) -> tuple:
    ...
    if zone == 'top3' and ctr < 0.15:           opp_type = 'ctr_fix'
    elif zone == 'first_page' and ctr < 0.05:   opp_type = 'ctr_fix'
    elif zone in ('push_zone_high', 'push_zone_mid'): opp_type = 'push'
    elif zone == 'content_gap':                  opp_type = 'content_gap'
    elif zone == 'deep_gap':                     opp_type = 'content_gap'
    elif impressions >= 50:                      opp_type = 'quick_win'
    else:                                        opp_type = 'low_priority'
        ...
        if city:
            if city not in city_gsc_data:
                city_gsc_data[city] = {
                    ...
                    'top_query': '', 'top_impressions': 0,
                    'opportunity_score': 0, 'opportunity_type': opp_type
                }
...
def analyze_diff(old_content: str, new_content: dict) -> dict:
    ...
    diff = {"is_new": False, "changes": [], "improvements": [], "metrics": {}}
...
def ts_lint_check(file_content: str, file_path: str) -> tuple:
    ...
    forbidden_patterns = [
        ("import type",    "Type imports forbidden"),
...
def validate_content(content: dict, city_slug: str) -> dict:
    ...
    suggested_fixes = []
    if "faq_missing"         in failure_codes: suggested_fixes.append("Add all 8 FAQ questions")
    if "schema_missing"      in failure_codes: suggested_fixes.append("Build JSON-LD FAQPage schema")
    if "eeat_missing"        in failure_codes: suggested_fixes.append("Add E-E-A-T footer")
    if "internal_links_low"  in failure_codes: suggested_fixes.append("Add /time/[city]/[other]/ links")
    if "thin_content"        in failure_codes: suggested_fixes.append("Expand content blocks to 150+ words")
    if "meta_desc_short"     in failure_codes: suggested_fixes.append("Write 130-160 char meta description")
    if "placeholder_text"    in failure_codes: suggested_fixes.append("Replace all [City] placeholders")
    return {
        "score": max(0, score), "issues": issues,
        "passed": score >= 70, "content": content,
        "failure_reason_codes": failure_codes, "suggested_fixes": suggested_fixes,
    }
...
STRATEGY: {"Improve title/meta for CTR" if gsc_data.get('opportunity_type') == 'ctr_fix' else "Strengthen content for ranking push" if gsc_data.get('opportunity_type') == 'push' else "Create comprehensive content"}
"""
    retry_ctx = ""
    if retry_issues:
...
{existing_ctx}{gsc_ctx}{retry_ctx}
...
{{
  "faq": [
    ...
    {{"question": "What is the time difference between {city_name} and New York?", "answer": "..."}},
    {{"question": "What is the time difference between {city_name} and London?", "answer": "..."}},
    {{"question": "What time does the stock exchange open in {city_name}?", "answer": "..."}},
    {{"question": "What is the best time to call {city_name} from the US?", "answer": "..."}},
    {{"question": "What are typical business hours in {city_name}?", "answer": "..."}}
  ],
  "faq_schema": {{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[]}},
  "seo_description": "150-160 char description",
  "seo_title": "Current Time in {city_name} — Timezone, UTC Offset & World Clock",
  "content_blocks": [
    {{"title": "{city_name} Time Zone Guide", "content": "150+ words..."}},
    {{"title": "Business Hours in {city_name}", "content": "150+ words..."}},
    {{"title": "Calling {city_name} Internationally", "content": "150+ words..."}}
  ],
...
REQUIREMENTS: All facts must be accurate. Replace ALL template text."""
...
async def process_city_full(city_slug: str, update, max_retries: int = 2) -> tuple:
    ...
    for attempt in range(max_retries + 1):
        ...
        seo_data, tokens, model = generate_city_seo(city_slug, existing, last_issues, gsc_data)
        tokens_total += tokens
        if not seo_data:
...
async def scheduled_friday_batch(app):
    ...
    if not chat_id:
        logger.warning("No TELEGRAM_CHAT_ID — skipping notification")
        return
        ...
        try:
            ...
            if seo_data:
                ...
                result = github_create_pr(
                    ...
                    city, f"Automated weekly SEO for {city}\nScore: {validation['score']}/100",
...
def run_scheduler(app):
    ...
    async def scheduler_loop():
        ...
        while state["scheduler_running"]:
            ...
            # ── SERP Trigger scan: her Pazartesi 06:05 ───────
            if now.weekday() == 0 and now.hour == 6 and 5 <= now.minute < 10 and TRIGGER_AVAILABLE:
                try:
                    global _trigger_candidates
                    rows = build_gsc_rows_for_trigger()
                    if rows:
                        _trigger_candidates = run_weekly_gsc_trigger(
                            TRIGGER_STORE_PATH,
                            rows,
                            out_path="candidates.json",
                        )
                        logger.info(f"Trigger scan: {len(_trigger_candidates)} candidates")
                except Exception as e:
                    logger.error(f"Trigger scan error: {e}")

            # ── SerpApi budget run: her sabah 07:00 ──────────
            if now.hour == 7 and now.minute < 5 and _trigger_candidates and TRIGGER_AVAILABLE:
                try:
                    decisions = run_daily_serp_decisions(
                        TRIGGER_STORE_PATH,
                        _trigger_candidates,
                        serp_fetch_for_trigger,
                        keyword_for_trigger_url,
                        daily_limit=8,
                        action_cooldown_days=14,
                    )
                    summary = build_action_summary(decisions)
                    logger.info(f"Trigger decisions: {summary['by_action']}")
                except Exception as e:
                    logger.error(f"Trigger decision error: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("""🚀 *Growth Agent v4.5*
GSC Connected | Scheduler | Diff Analyzer | SERP Trigger
...
*SERP Trigger:*
/trigger — Haftalık scan (hangi şehir aksiyon adayı)
/triggerrun — SerpApi budget run (en öncelikli adaylar)
/triggerstatus — Mevcut aday listesi
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trig_ok  = "✅" if TRIGGER_AVAILABLE else "❌ (serp_trigger_batch.py yok)"
    last_gsc = state.get("last_gsc_refresh", "Henüz yok")
    if last_gsc and last_gsc != "Henüz yok":
        last_gsc = last_gsc[:16].replace('T', ' ')
    cand_count = len(_trigger_candidates)
    await update.message.reply_text(
        f"📊 *Growth Agent v4.5 — Durum*\n\n"
        f"🌐 Site: `{state['active_site']}`\n"
        f"🔌 GSC: {gsc_ok}\n"
        f"⏰ Scheduler: {sched_ok}\n"
        f"🔄 Auto batch: {'✅' if state['auto_mode'] else '❌'}\n"
        f"🎯 SERP Trigger: {trig_ok}\n"
        f"📋 Trigger candidates: {cand_count}\n"
        f"🕐 Son GSC refresh: {last_gsc}\n\n"
        f"*Bu Session:*\n"
        f"✅ {state['processed']} şehir | ❌ {state['failed']} fail\n"
        f"🔤 {state['token_used']:,} token | 💰 ${cost:.4f} ({pct:.1f}%)\n\n"
        f"*Toplam:*\n"
        f"📈 {stats['processed_cities']}/{stats['total_cities']} şehir | ⭐ {stats['avg_score']}/100\n"
        f"🔑 {stats['gsc_keywords']} GSC keyword\n\n"
        f"⚙️ `{config.get('GITHUB_REPO','?')}` → `{config.get('GITHUB_BRANCH','?')}`"
    )
...
async def gscstats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute('''SELECT query, impressions, clicks, ctr, position, opportunity_type
                 FROM gsc_keywords WHERE city_slug=? ORDER BY impressions DESC LIMIT 5''', (city_slug,))
    keywords = c.fetchall()
    conn.close()
    city_name = city_slug.replace('-', ' ').title()
    ...
    lines = [
        ...
        f"📈 Impressions: {imp:,}", f"👆 Clicks: {clk:,}",
        f"📊 CTR: {ctr*100:.1f}%", f"📍 Avg Position: {pos:.1f}",
        f"🔑 Top Query: {top_q}", f"🎯 Priority Score: {priority:.0f}/100", "",
        f"Top Keywords:",
    ]
    for kw in keywords:
        q, i, cl, ct, p, ot = kw
        lines.append(f"• {q[:40]} — {i} imp | pos {p:.0f} | {ot}")
    lines += ["", f"Son güncelleme: {(updated or 'N/A')[:16]}"]
    await update.message.reply_text("\n".join(lines))

# ── SERP Trigger Telegram komutları ─────────────────────────

async def trigger_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Manuel trigger scan — hangi şehir aksiyon adayı?"""
    if not TRIGGER_AVAILABLE:
        await update.message.reply_text("❌ serp_trigger_batch.py bulunamadı.")
        return
    await update.message.reply_text("🔍 Trigger scan başlıyor (2 haftalık GSC delta)...")
    global _trigger_candidates
    rows = build_gsc_rows_for_trigger()
    if not rows:
        await update.message.reply_text("❌ GSC verisi yok. Önce /gsc çalıştır.")
        return
    _trigger_candidates = run_weekly_gsc_trigger(
        TRIGGER_STORE_PATH, rows, out_path="candidates.json"
    )
    from collections import Counter
    type_counts = Counter(c.candidate_type for c in _trigger_candidates)
    top_check   = [c for c in _trigger_candidates if c.candidate_type == "SERP_CHECK"][:5]
    top_ctr     = [c for c in _trigger_candidates if c.candidate_type == "CTR_OPT"][:5]
    lines = [
        "✅ *Trigger Scan Tamamlandı*", "",
        f"🔴 SERP_CHECK: {type_counts.get('SERP_CHECK', 0)} şehir",
        f"🟡 CTR_OPT:    {type_counts.get('CTR_OPT', 0)} şehir",
        f"🟢 HOLD:       {type_counts.get('HOLD', 0)} şehir", "",
    ]
    if top_check:
        lines.append("*Top SERP_CHECK:*")
        for c in top_check:
            slug = c.url.split("/")[-1]
            lines.append(f"  {slug} | pos {c.gsc['pos_7d']:.1f} (Δ{c.gsc['dPos']:+.1f}) | {c.reasons[0][:50]}")
    if top_ctr:
        lines.append("*Top CTR_OPT:*")
        for c in top_ctr:
            slug = c.url.split("/")[-1]
            lines.append(f"  {slug} | pos {c.gsc['pos_7d']:.1f} | CTR {c.gsc['ctr_7d']:.2f}%")
    lines += ["", "SerpApi budget için: /triggerrun"]
    await update.message.reply_text("\n".join(lines))

async def triggerrun_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """SerpApi budget run — en öncelikli adaylara sorgu at, action kararı ver."""
    if not TRIGGER_AVAILABLE:
        await update.message.reply_text("❌ serp_trigger_batch.py bulunamadı.")
        return
    if not _trigger_candidates:
        await update.message.reply_text("❌ Önce /trigger çalıştır.")
        return
    await update.message.reply_text("⚡ SerpApi budget run başlıyor...")
    decisions = run_daily_serp_decisions(
        TRIGGER_STORE_PATH,
        _trigger_candidates,
        serp_fetch_for_trigger,
    ...
    summary = build_action_summary(decisions)
    icons   = {
        "MINOR_OPTIMIZE": "🔧", "SNIPPET_TARGET": "⭐",
        "AUTHORITY_PUSH": "🏋️", "CTR_TEST": "📝",
        "HOLD": "✋", "MAJOR_REWRITE": "🔄",
    }
    lines = ["✅ *SerpApi Budget Run Tamamlandı*", f"Toplam karar: {summary['total']}", ""]
    for action, count in sorted(summary["by_action"].items(), key=lambda x: -x[1]):
        icon = icons.get(action, "•")
        lines.append(f"{icon} {action}: {count}")
    lines += ["", f"HOLD oranı: %{summary['hold_pct']}"]
    for d in summary["decisions"][:5]:
        if d["action"] != "HOLD":
            slug = d["url"].split("/")[-1]
            lines.append(f"\n`{slug}` → {d['action']} [{d['size']}]")
            if d["notes"]:
                lines.append(f"  {d['notes'][0][:60]}")
    await update.message.reply_text("\n".join(lines)[:3500])

async def triggerstatus_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Mevcut trigger candidate listesini göster."""
    if not _trigger_candidates:
        await update.message.reply_text("📋 Trigger candidates yok. /trigger çalıştır.")
        return
    from collections import Counter
    type_counts = Counter(c.candidate_type for c in _trigger_candidates)
    lines = [
        f"📋 *Trigger Candidates* ({len(_trigger_candidates)} şehir)", "",
        f"🔴 SERP_CHECK: {type_counts.get('SERP_CHECK', 0)}",
        f"🟡 CTR_OPT:    {type_counts.get('CTR_OPT', 0)}",
        f"🟢 HOLD:       {type_counts.get('HOLD', 0)}", "",
        "*Top 10 (non-HOLD):*"
    ]
    non_hold = [c for c in _trigger_candidates if c.candidate_type != "HOLD"][:10]
    for c in non_hold:
        slug = c.url.split("/")[-1]
        lines.append(
            f"  {slug:20s} | {c.candidate_type:12s} | "
            f"pos {c.gsc['pos_7d']:5.1f} | score {c.priority:.1f}"
        )
    await update.message.reply_text("\n".join(lines))
...
async def run_city_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        seo_data, validation, diff, tokens, model = await process_city_full(city_slug, update, max_retries=2)
        if not seo_data:
            state["failed"] += 1
        ...
        if not lint_passed:
            await update.message.reply_text(f"TS Lint FAILED — PR blocked\n\n" + "\n".join(lint_issues[:5]))
            state["failed"] += 1
            return
...
**Growth Agent v4.5** | {datetime.now().strftime('%Y-%m-%d %H:%M')} | {model}
...
- [ ] Internal links working"""
        result = github_create_pr(
            ...
            city_slug, pr_body, {f"data/seo/{city_slug}-seo.ts": tsx}
        )
        if result.startswith("SUCCESS"):
            state["processed"] += 1
            pr_url = result.replace("SUCCESS: ", "")
            db_record_processing(city_slug, validation["score"], pr_url, validation["issues"],
                                  model, tokens, seo_data.get("improvement_notes", "")[:200])
            await update.message.reply_text(
                f"✅ *{city_name}* tamamlandı!\n\n"
                f"⭐ Skor: {validation['score']}/100\n"
                f"📝 {diff.get('summary', '')[:120]}\n"
...
async def batch_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, city in enumerate(cities):
        ...
        try:
            ...
            if seo_data:
                ...
                result = github_create_pr(
                    f"SEO v4: {city_name} — {validation['score']}/100",
                    city, f"Batch SEO for {city_name}\nScore: {validation['score']}/100",
                    {f"data/seo/{city}-seo.ts": tsx}
                )
                if result.startswith("SUCCESS"):
                    success += 1
    ...
    cost = state["token_used"] * 0.000003
    await update.message.reply_text(f"🎉 *Batch tamamlandı!*\n✅ {success}/{n} | 💰 ~${cost:.4f}")
...
async def auto_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["auto_mode"]:
        await update.message.reply_text(
            ...
            "🎯 Pazartesi 06:05 → SERP trigger scan\n"
            "⚡ Her gün 07:00 → SerpApi budget run\n\n"
    ...
    try:
        ...
        for m in matches:
            ...
            c.execute('''INSERT OR IGNORE INTO city_priority 
                         ...
                         VALUES (?, ?, ?, ?, ?)''', (slug, base_score, tier, continent, now))
            count += 1
        conn.commit()
        conn.close()
        await update.message.reply_text(
...
async def history_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in history:
        ...
        if pr:    msg += f"🔗 {pr}\n"
        if issues: msg += f"⚠️ {', '.join(issues[:2])}\n"
        msg += "\n"
...
Give: 3 priorities, 5 keyword opportunities, 2 competitor gaps, 1 quick win. Agency level."""
    try:
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("""📖 *v4.5 Komutlar*
...
async def lessons_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if retry_stats and retry_stats[2]:
        msg += f"\nRetry Stats: avg {retry_stats[0]:.1f}, max {retry_stats[1]}, total {retry_stats[2]} retried"
    else:
        msg += "\nNo retry data yet"
    await update.message.reply_text(msg[:3500])
...
def main():
    ...
    if not token:
        ...
        return
    logger.info(f"Growth Agent v4.5 starting — site: {state['active_site']}, trigger: {TRIGGER_AVAILABLE}")
    ...
    handlers = [
        ...
        ("lessons",        lessons_cmd),
        ("help",           help_cmd),
        # SERP Trigger komutları
        ("trigger",        trigger_cmd),
        ("triggerrun",     triggerrun_cmd),
        ("triggerstatus",  triggerstatus_cmd),
    ]
    ...
    logger.info("Growth Agent v4.5 active!")
