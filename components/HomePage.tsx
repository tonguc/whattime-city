            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <div className="flex-shrink-0 flex justify-center items-center md:flex-[2] min-h-[70px] md:min-h-[80px]">
                ...
                {clockMode === 'analog' ? (
                  ...
                  <div className={`text-5xl md:text-6xl font-mono font-bold tracking-tight mr-5 whitespace-nowrap tabular-nums ${text}`}>
            ...
            <div className="flex flex-wrap gap-2">
              {favoriteCities.slice(0, 6).map(city => (
                <Link key={city.slug} href={`/${city.slug}`}
                  ...
                  <span className={`ml-2 font-mono tabular-nums ${textMuted}`}>{getLocalTime(city)}</span>
                </Link>
              ))}
            </div>
          </section>
              ...
              return (
                <div key={city.slug} className={`flex items-center gap-4 p-4 rounded-xl ${isLight ? 'bg-slate-50' : 'bg-slate-800/50'}`}>
                  ...
                  <div className="text-right">
                    <div className={`text-xl font-mono font-bold tabular-nums ${text}`}>{cityTime}</div>
