...
import { City, getTier1Cities, uses12HourFormat, cities, getCitiesByContinent, Continent, continentLabels, getCitiesByCountryCode, getCountryBySlug } from '@/lib/cities'
    ...
    <div className={`min-h-screen bg-gradient-to-br ${theme.bg} transition-all duration-1000 relative`}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 py-4 sm:py-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
          ...
          {/* Paris Guide Card - only for Paris */}
          {selectedCity.slug === 'paris' && (
            <a 
              href="/paris/guide/"
              className={`group rounded-2xl p-4 flex items-center gap-3 col-span-2 md:col-span-4 transition-all hover:scale-[1.01] ${
                isLight 
            ...
            >
              ...
              <div className="flex-1">
                <div className={`font-bold ${isLight ? 'text-slate-800' : 'text-white'}`}>Paris Time Guide</div>
                <div className={`text-sm ${isLight ? 'text-slate-600' : 'text-slate-300'}`}>
                  Euronext hours, café culture, French holidays & more
                </div>
              </div>
              <div className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                isLight 
