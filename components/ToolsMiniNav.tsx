...
const toolNavItems = [
  {
    ...
    name: 'Time Converter',
    url: '/time',
    icon: (<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>)
  },
  {
    id: 'meeting-planner',
  ...
  },
  {
    ...
    url: '/jet-lag-advisor',
    icon: (<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 1 0 10 10 4 4 0 0 1-5-5 4 4 0 0 1-5-5"/><path d="M8.5 8.5v.01"/><path d="M16 15.5v.01"/></svg>)
  },
  {
    id: 'event-time',
    name: 'Event Time',
    url: '/event-time',
    icon: (<svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 10"/></svg>)
  }
]

const EXCLUDED_SLUGS = [
  'meeting','time','flight-time','jet-lag-advisor','event-time','time-converter',
  'cities','guides','map','tools','countries','world-alarm','world-clock',
  'search','about','privacy','contact'
]
    ...
    <nav className="mt-2 mb-6 -mx-4 px-4">
      <div className="flex sm:justify-center gap-2 overflow-x-auto scrollbar-hide pb-2 sm:pb-0 sm:overflow-visible sm:flex-wrap">
        {toolNavItems.map((tool) => {
          ...
          const href =
            tool.id === 'meeting-planner' && citySlug
              ? \`/meeting/\${citySlug}\`
              : tool.url
          ...
          return (
            <Link
              key={tool.id}
              href={href}
              onClick={tool.id === 'meeting-planner' ? handleMeetingClick : undefined}
              className={className}
            >
