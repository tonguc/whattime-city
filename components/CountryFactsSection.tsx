...
export default function CountryFactsSection({ hubSlug }: Props) {
  ...
  // Multi-TZ detection: country has cities in more than 1 unique IANA timezone
  const uniqueIanaTzs = Array.from(new Set(cities.map(c => c.timezone)))
  const isMultiTz = uniqueIanaTzs.length > 1

  // For multi-TZ: one representative city per unique timezone (best tier)
  const tzGroups = uniqueIanaTzs.map(tz => {
    const group = cities
      .filter(c => c.timezone === tz)
      .sort((a, b) => (a.tier ?? 3) - (b.tier ?? 3))
    return { tz, city: group[0] }
  }).filter(g => g.city)

  // Call window table
  const REF_ZONES = [
    { tz: 'America/New_York',    from: 'EST / EDT (New York)'    },
    { tz: 'America/Los_Angeles', from: 'PST / PDT (Los Angeles)' },
    { tz: 'Europe/London',       from: 'GMT / BST (London)'      },
    { tz: 'Europe/Paris',        from: 'CET / CEST (Paris)'      },
    { tz: 'Asia/Dubai',          from: 'GST (Dubai)'             },
  ]
  const getCallRow = (refTz: string) => {
    ...
    if (winRefStart >= winRefEnd) return null
  ...
  const CurrentTimeAcrossCountry = isMultiTz ? (
    <section className={card}>
      <h2 className={`text-lg font-semibold mb-3 ${heading}`}>
        Current Time Across {country.name}
      </h2>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className={`border-b ${tableBorder} ${muted}`}>
              <th className="text-left py-2 pr-4 font-medium">City / Region</th>
              <th className="text-left py-2 pr-4 font-medium">Time Zone</th>
              <th className="text-left py-2 font-medium">Current Time</th>
            </tr>
          </thead>
          <tbody className={`divide-y ${tableDivide}`}>
            {tzGroups.map(({ tz, city }) => (
              <tr key={tz}>
                <td className={`py-2 pr-4 font-medium ${heading}`}>
                  <Link href={`/${city.slug}`} className="hover:underline">{city.city}</Link>
                </td>
                <td className={`py-2 pr-4 ${muted}`}>{getTzAbbr(tz)}</td>
                <td className={`py-2 ${timeGreen}`}>{getTime(tz)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  const TimeDifference = (
    <ClockComparisonSection primaryTz={primaryTz} countryName={country.name} />
  )
          ...
          {cities.map(city => (
            <Link key={city.slug} href={`/${city.slug}`} className={cityLink}>
              <div>
                ...
                <div className={`text-xs ${muted}`}>{city.timezone.split('/').pop()?.replace(/_/g, ' ')}</div>
              </div>
  ...
  const OtherCountries = relatedCountries.length > 0 ? (
    <section className={card}>
