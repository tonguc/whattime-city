...
export default function WorldMap() {
  ...
  const handleReset = () => { setZoom(1); setFocusedCity(null) }
  ...
  // Stable getCityData — only changes when cityDataMap changes (once/minute)
  const getCityData = useCallback((slug: string) => {
    const city = cities.find(c => c.slug === slug)
    if (!city) return null
    const data = cityDataMap.get(slug)
    if (!data) return null
    const { timeStr, timeOfDay } = data
    const theme = themes[timeOfDay as keyof typeof themes]
    return { city, timeStr, timeOfDay, theme }
  }, [cityDataMap])

  // Tab counts — derived from pre-computed map, not recalculated every render
  const tabCounts = useMemo(() => {
    const counts = { dawn: 0, day: 0, dusk: 0, night: 0 }
    cityDataMap.forEach((data) => {
      if (data.timeOfDay in counts) counts[data.timeOfDay as keyof typeof counts]++
    })
    return counts
  }, [cityDataMap])

  // Continent counts by active time tab
  const continentCounts = useMemo(() => {
    const filtered = activeTimeTab === 'all'
      ? cities
      : cities.filter(c => cityDataMap.get(c.slug)?.timeOfDay === activeTimeTab)
    const counts: Record<string, number> = { all: filtered.length }
    for (const city of filtered) {
      if (city.continent) counts[city.continent] = (counts[city.continent] ?? 0) + 1
    }
    return counts
  }, [cityDataMap, activeTimeTab]) // eslint-disable-line react-hooks/exhaustive-deps

  // Filtered + sorted city list for grid
  const filteredCities = useMemo(() => {
    return cities
      .filter(city => {
        if (activeTimeTab !== 'all' && cityDataMap.get(city.slug)?.timeOfDay !== activeTimeTab) return false
        if (activeContinentTab !== 'all' && city.continent !== activeContinentTab) return false
        return true
      })
      .sort((a, b) => a.city.localeCompare(b.city))
  }, [cityDataMap, activeTimeTab, activeContinentTab]) // eslint-disable-line react-hooks/exhaustive-deps

  // Reset pagination when filters change
  useEffect(() => { setGridPage(1) }, [activeTimeTab, activeContinentTab])

  const visibleGridCities = filteredCities.slice(0, gridPage * GRID_PAGE_SIZE)
  const hasMore = visibleGridCities.length < filteredCities.length
      ...
      <div className="relative max-w-6xl mx-auto px-4 py-8">
          ...
          {showSearchResults && searchResults.length > 0 && (
            <div className={`absolute top-full left-0 right-0 mt-2 rounded-2xl overflow-hidden backdrop-blur-xl border z-50 ${
              ...
              {searchResults.map((city) => {
                const data = cityDataMap.get(city.slug)
                return (
                  <button
                    key={city.slug}
                  ...
                  >
                    <div className="text-left">
                      <div className={`font-medium ${text}`}>{city.city}</div>
                      <div className={`text-sm ${textMuted}`}>{city.country}</div>
                    </div>
        ...
        <div className={`relative rounded-3xl overflow-hidden backdrop-blur-xl border ${
          ...
          <div className="absolute top-4 right-4 z-20 flex flex-col gap-2">
            <button onClick={handleZoomIn} className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl font-bold transition-all ${
              isLight ? 'bg-white/80 text-slate-700 hover:bg-white shadow-lg' : 'bg-slate-700/80 text-white hover:bg-slate-600 shadow-lg'
            }`}>+</button>
            <button onClick={handleZoomOut} className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl font-bold transition-all ${
              isLight ? 'bg-white/80 text-slate-700 hover:bg-white shadow-lg' : 'bg-slate-700/80 text-white hover:bg-slate-600 shadow-lg'
            }`}>−</button>
            {zoom > 1 && (
              <button onClick={handleReset} className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-medium transition-all ${
                isLight ? 'bg-white/80 text-slate-700 hover:bg-white shadow-lg' : 'bg-slate-700/80 text-white hover:bg-slate-600 shadow-lg'
              }`}>↺</button>
            )}
          </div>
            ...
            return (
              <div className={`absolute bottom-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-64 p-4 rounded-2xl backdrop-blur-xl border ${
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className={`text-xs uppercase tracking-wide ${textMuted}`}>{city.country}</div>
                    <div className={`text-lg font-bold ${text}`}>{city.city}</div>
                  </div>
                  <div className="text-right">
                    <div className={`text-2xl font-bold ${text}`}>{timeStr}</div>
                ...
                <Link
                  ...
                  className={`mt-3 block w-full text-center py-2 rounded-full text-sm font-medium transition-all ${
                    isLight ? 'bg-slate-800 text-white hover:bg-slate-700' : 'bg-white text-slate-800 hover:bg-slate-100'
                  }`}
                >
                  View Details →
                </Link>
        ...
        <div className={`mt-6 flex flex-wrap justify-center gap-6 text-sm ${textMuted}`}>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-yellow-400"></span><span>Daytime</span></div>
          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-cyan-400"></span><span>Nighttime</span></div>
          <div className="flex items-center gap-2"><span className="w-6 border-t-2 border-dashed border-yellow-500"></span><span>Day/Night Line</span></div>
        </div>

        {/* SEO Content Section */}
        <div className={`mt-8 rounded-2xl p-6 ${isLight ? 'bg-white/60' : 'bg-slate-800/40'}`}>
          <h2 className={`text-xl font-semibold mb-4 ${text}`}>Interactive World Time Zone Map</h2>
          <div className={`space-y-4 ${textMuted}`}>
            ...
            <h3 className={`text-lg font-medium mt-4 ${isLight ? 'text-slate-700' : 'text-white'}`}>Understanding Time Zones</h3>
            <p>
            ...
            <h3 className={`text-lg font-medium mt-4 ${isLight ? 'text-slate-700' : 'text-white'}`}>Day and Night Visualization</h3>
            <p>
        ...
        {/* City Grid with Tabs */}
        <div className="mt-8">
          <h2 className={`text-xl font-semibold mb-4 ${isLight ? 'text-slate-700' : 'text-white'}`}>World Cities</h2>

          {/* Time of Day Tabs */}
          <div className="mb-4">
            <div className={`text-sm font-medium mb-2 ${textMuted}`}>Filter by Time of Day</div>
            <div className="flex flex-wrap gap-2">
              {([
                ...
                const count = tab.id === 'all' ? cities.length : tabCounts[tab.id]
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                      isActive
                        ? isLight ? 'bg-slate-800 text-white' : 'bg-white text-slate-800'
          ...
          {/* Continent Tabs */}
          <div className="mb-6">
            <div className={`text-sm font-medium mb-2 ${textMuted}`}>Filter by Region</div>
            <div className="flex flex-wrap gap-2">
              {([
                ...
                const count = continentCounts[tab.id] ?? 0
                const isActive = activeContinentTab === tab.id
                return (
                  <button
                    key={tab.id}
                    onClick={() => setActiveContinentTab(tab.id)}
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                      isActive
                        ? isLight ? 'bg-slate-800 text-white' : 'bg-white text-slate-800'
                        : isLight ? 'bg-white/60 text-slate-600 hover:bg-white/80' : 'bg-slate-800/60 text-slate-300 hover:bg-slate-700/60'
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2">
            {visibleGridCities.map(city => {
              const data = cityDataMap.get(city.slug)
              const timeStr = data?.timeStr ?? '--:--'
              const timeOfDay = data?.timeOfDay ?? 'day'
              const timeColor =
                timeOfDay === 'night' ? 'text-cyan-400' :
                timeOfDay === 'dusk'  ? 'text-orange-400' :
              ...
              return (
                ...
                >
                  ...
                  <div className={`font-medium truncate ${text}`}>{city.city}</div>
                  <div className={`text-lg font-bold ${timeColor}`}>{timeStr}</div>
                </Link>
              )
            })}
          </div>
          ...
          {filteredCities.length === 0 && (
            <div className={`text-center py-12 ${textMuted}`}>No cities match the selected filters</div>
          )}
          ...
          {hasMore && (
            <div className="mt-6 text-center">
              ...
              >
                Show more ({filteredCities.length - visibleGridCities.length} remaining)
              </button>
            </div>
          )}
        </div>
        ...
        <div className={`mt-8 rounded-2xl p-6 ${isLight ? 'bg-white/60' : 'bg-slate-800/40'}`}>
          <h2 className={`text-xl font-semibold mb-4 ${text}`}>How to Use the World Clock Map</h2>
          <div className={`space-y-4 ${textMuted}`}>
            <p>
