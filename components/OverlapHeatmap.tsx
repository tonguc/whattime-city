              ...
              <>
                ...
                {tooltip.allAwake ? (
                  ...
                  <div className={`flex items-center gap-2 mb-2 ${
                    ...
                    <span className="font-medium">No good overlap</span>
                  </div>
                ) : (
                  <div className={`flex items-center gap-2 mb-2 ${
      ...
      {bestOverlap && (
        <div className={`mt-4 p-4 rounded-xl ${
          ...
          <div className="flex items-start gap-3">
            {bestOverlap.score === 1 ? (
              <svg className={`w-5 h-5 mt-0.5 ${isLight ? 'text-green-600' : 'text-green-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            ) : (
            ...
            <div>
              <div className={`font-semibold ${
                ...
                {bestOverlap.score === 1 
                  ? `${bestOverlap.duration} hour${bestOverlap.duration > 1 ? 's' : ''} of full overlap`
                  : `Best available: ${Math.round(bestOverlap.score * 100)}% overlap`
                }
              </div>
              <div className={`text-sm mt-0.5 ${
                ...
                Recommended time: {formatHour(bestOverlap.startHour)} - {formatHour(bestOverlap.endHour + 1)} ({refCityName} time)
              </div>
              {bestOverlap.score < 1 && (
                <div className={`text-xs mt-1 ${isLight ? 'text-amber-500' : 'text-amber-500'}`}>
                  Some participants may need to compromise on timing
                </div>
              )}
            </div>
          </div>
