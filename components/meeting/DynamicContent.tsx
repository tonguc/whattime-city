...
export default function DynamicContent({ city1, city2 }: Props) {
  ...
  return (
    <div className="prose prose-slate dark:prose-invert max-w-none">
      ...
      <section className="mb-8">
        <h3 className="text-xl font-semibold mb-3">Quick Reference Table</h3>
        <div className="overflow-x-auto">
          <table className="min-w-full border-collapse">
            <thead>
              <tr className="border-b-2 border-slate-300 dark:border-slate-700">
                <th className="text-left py-2 px-4">{city1.city} Time</th>
                <th className="text-left py-2 px-4">{city2.city} Time</th>
      ...
      <section className="mb-8">
        ...
        <ul className="space-y-2">
          {bestWindows.length > 0 && (
            <li>
              <strong>Best time window:</strong> {bestWindows.slice(0, 2).join(' or ')}
            </li>
          )}
          <li>
            <strong>Avoid:</strong> {avoidTimes}
          </li>
          {timeDiff.hours >= 10 && (
            <li>
              <strong>Alternative approach:</strong> Due to the large time difference, consider recording meetings for async review or rotating meeting times to share the inconvenience fairly between teams.
            </li>
          )}
          <li>
            <strong>Planning ahead:</strong> Send meeting invites with times in both time zones to avoid confusion. Tools like calendar apps automatically convert times.
          </li>
        </ul>
      </section>
      ...
      <section className="mb-8">
        <h3 className="text-xl font-semibold mb-3">Business Hours Comparison</h3>
        <div className="grid md:grid-cols-2 gap-4">
          <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <h4 className="font-semibold mb-2">{city1.city}</h4>
            <p className="text-sm">
              <strong>Timezone:</strong> {city1.timezone}<br/>
              <strong>Business hours:</strong> 9:00 AM - 5:00 PM<br/>
              <strong>When {city2.city} starts work (9 AM):</strong> {formatHour(getLocalHour(city1, 9))} in {city1.city}
            </p>
          </div>
          <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg">
            <h4 className="font-semibold mb-2">{city2.city}</h4>
            <p className="text-sm">
              <strong>Timezone:</strong> {city2.timezone}<br/>
              <strong>Business hours:</strong> 9:00 AM - 5:00 PM<br/>
              <strong>When {city1.city} starts work (9 AM):</strong> {formatHour(getLocalHour(city2, 9))} in {city2.city}
            </p>
          </div>
        </div>
      ...
      {/* Common Use Cases */}
      <section className="mb-8">
        <h3 className="text-xl font-semibold mb-3">Common Scenarios</h3>
        <div className="space-y-3">
          <div>
            <h4 className="font-medium">Daily standups</h4>
          ...
          <div>
            <h4 className="font-medium">Client calls</h4>
            <p className="text-sm text-slate-600 dark:text-slate-400">
          ...
          <div>
            <h4 className="font-medium">Emergency coordination</h4>
            <p className="text-sm text-slate-600 dark:text-slate-400">
      ...
      <section>
        ...
        <div className="space-y-4">
          <div>
            <h4 className="font-medium">Does the time difference account for Daylight Saving Time?</h4>
            <p className="text-sm text-slate-600 dark:text-slate-400">
          ...
          <div>
            <h4 className="font-medium">What if I need to add more participants?</h4>
            <p className="text-sm text-slate-600 dark:text-slate-400">
          ...
          <div>
            <h4 className="font-medium">Can I share this meeting plan with my team?</h4>
            <p className="text-sm text-slate-600 dark:text-slate-400">
